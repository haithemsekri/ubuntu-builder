...
if [ "$TARGET_NAME" == "opipc2" ]; then
   ...
   [ -z $KERNEL_DOM0_BOOT_FILE ] && KERNEL_DOM0_BOOT_FILE="$SCRIPTS_DIR/32-kernel-boot-env-opipc2.cmd"
   [ -z $KERNEL_DOM0_CONFIG_FILE ] && KERNEL_DOM0_CONFIG_FILE="$SCRIPTS_DIR/32-kernel-config-dom0-opipc2.config"
